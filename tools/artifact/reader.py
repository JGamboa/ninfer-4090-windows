...
class Artifact:
    ...
    def __init__(self, path: str | Path) -> None:
        ...
        try:
            fd = os.open(self.path, os.O_RDONLY | os_compat.BINARY_FLAG)
            self._fds[0] = fd
            entry_bytes = os.fstat(fd).st_size
            raw = os_compat.pread(fd, HEADER.size, 0)
            ...
            raw_json = os_compat.pread(fd, json_bytes, HEADER.size)
    ...
    def iter_range(
        ...
        while offset < end:
            ...
            fd = self._file(index)
            data = os_compat.pread(fd, count, file_offset)
            if len(data) != count:
                raise ArtifactError(
