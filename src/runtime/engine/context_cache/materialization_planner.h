...
#ifdef _WIN32
            // MSVC lacks __uint128_t: compare the two 64x64 products via their 128-bit
            // (hi,lo) halves computed with _umul128.
            std::uint64_t left_hi = 0, right_hi = 0;
            const std::uint64_t left_lo  = _umul128(static_cast<std::uint64_t>(delta(cost)), b, &left_hi);
            const std::uint64_t right_lo = _umul128(static_cast<std::uint64_t>(delta(prior)), a, &right_hi);
            if (left_hi != right_hi) { return left_hi < right_hi; }
            if (left_lo != right_lo) { return left_lo < right_lo; }
#else
