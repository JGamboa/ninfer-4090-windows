...
#ifdef _WIN32
// MSVC lacks unsigned __int128. Provide a minimal 128-bit unsigned helper supporting exactly the
// operations used below: multiplication of two 64-bit values, add/sub of small constants, left
// shift, right shift, and comparison. The high/low product uses the _umul128 intrinsic.
struct U128 {
    std::uint64_t low  = 0;
    std::uint64_t high = 0;

    U128() = default;
    U128(std::uint64_t value) noexcept : low(value), high(0) {}
    U128(std::uint64_t high_part, std::uint64_t low_part) noexcept : low(low_part), high(high_part) {}

    friend U128 operator*(U128 left, std::uint64_t right) noexcept {
        // Only ever called with a 64-bit left operand (high == 0).
        std::uint64_t high = 0;
        const std::uint64_t low = _umul128(left.low, right, &high);
        return U128{high, low};
    }

    friend U128 operator+(U128 value, std::uint64_t addend) noexcept {
        const std::uint64_t low = value.low + addend;
        const std::uint64_t carry = low < value.low ? 1U : 0U;
        return U128{value.high + carry, low};
    }

    friend U128 operator-(U128 value, std::uint64_t sub) noexcept {
        const std::uint64_t low = value.low - sub;
        const std::uint64_t borrow = value.low < sub ? 1U : 0U;
        return U128{value.high - borrow, low};
    }

    friend U128 operator<<(U128 value, unsigned shift) noexcept {
        if (shift == 0) { return value; }
        if (shift >= 64) { return U128{value.low << (shift - 64), 0}; }
        return U128{(value.high << shift) | (value.low >> (64 - shift)), value.low << shift};
    }

    [[nodiscard]] std::uint64_t operator>>(unsigned shift) const noexcept {
        // The results here always fit in 64 bits after the shift used by callers.
        if (shift == 0) { return low; }
        if (shift >= 64) { return high >> (shift - 64); }
        return (high << (64 - shift)) | (low >> shift);
    }

    friend bool operator>(U128 left, std::uint64_t right) noexcept {
        return left.high != 0 || left.low > right;
    }

    friend bool operator>=(U128 left, U128 right) noexcept {
        return left.high != right.high ? left.high > right.high : left.low >= right.low;
    }

    explicit operator std::uint64_t() const noexcept { return low; }
};
#else
