...
[[nodiscard]] inline PrefillWork make_prefill_work(std::uint64_t prefix_tokens,
    ...
    // Attention work is prefix*suffix + suffix*(suffix+1)/2, saturating at u64 max. Compute each
    // 64x64->128 product portably (MSVC lacks unsigned __int128) and saturate on any overflow.
    constexpr std::uint64_t u64_max = std::numeric_limits<std::uint64_t>::max();
#ifdef _WIN32
    // 64x64 -> 128 multiply via intrinsic; returns saturated u64 result via out-params.
    const auto mul_hi_lo = [](std::uint64_t a, std::uint64_t b, std::uint64_t& hi) -> std::uint64_t {
        return _umul128(a, b, &hi);
    };
    std::uint64_t linear_hi   = 0;
    const std::uint64_t linear_lo = mul_hi_lo(prefix_tokens, suffix_tokens, linear_hi);
    // triangular = suffix*(suffix+1)/2. suffix*(suffix+1) is even, so the 128-bit product's
    // low bit is 0; a 128-bit right shift by 1 is exact.
    std::uint64_t tri_hi          = 0;
    const std::uint64_t tri_lo_raw = mul_hi_lo(suffix_tokens, suffix_tokens + 1U, tri_hi);
    const std::uint64_t tri_lo    = (tri_lo_raw >> 1U) | (tri_hi << 63U);
    tri_hi >>= 1U;

    std::uint64_t attention = u64_max;
    if (linear_hi == 0 && tri_hi == 0) {
        const std::uint64_t sum = linear_lo + tri_lo;
        attention = sum < linear_lo ? u64_max : sum; // carry out of 64 bits => saturate
    }
    ...
    result.attention_pairs = attention > u64_max ? u64_max : static_cast<std::uint64_t>(attention);
#endif
    return result;
}
