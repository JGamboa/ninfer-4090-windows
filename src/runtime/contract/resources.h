...
[[nodiscard]] inline PrefillWork make_prefill_work(std::uint64_t prefix_tokens,
    ...
    const std::uint64_t linear_lo = _umul128(prefix_tokens, suffix_tokens, &linear_hi);
    ...
    const std::uint64_t tri_lo_raw = _umul128(suffix_tokens, suffix_tokens + 1U, &tri_hi);
