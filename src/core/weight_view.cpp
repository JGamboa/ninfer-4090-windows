...
WeightGeometry weight_geometry(QType format, QuantLayout layout,
    ...
    if (layout == QuantLayout::RowSplit) {
        ...
        // Ternary code rows (code c means c - 1) of 13-byte scaled base-3 units of 64 columns
        // (T5_G128_FP16), then FP16 scales per 128 columns.
        if (format != QType::T5_G128_FP16 || k % 128) {
            throw std::invalid_argument("TernaryRowK128 requires T5_G128_FP16 and K%128=0");
        }
        out.group_size          = 128;
        out.code_bytes_per_row  = k / 64 * 13;
