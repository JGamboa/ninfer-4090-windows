...
// The packed Q/K/gate/V parent of a Q5 MTP layer. Every route takes its row count at run time,
// so this is the n7168 table over twice the rows (the same compiled instances); its bounds
// are not retuned for this geometry.
Q5Launch select_q5_n14336_k5120(std::int32_t tokens) {
