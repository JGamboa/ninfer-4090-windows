...
// The W4A4 routes need block-scaled mma (.kind::mxf4nvf4, sm_100+) and TMA; on sm_86/sm_89 the
// shape files still select them by token count, so hand back a launch that refuses instead of
// instantiating kernels ptxas cannot assemble for this target. Startup already rejects NVFP4
// artifacts on these parts, so this is unreachable in practice.
inline void launch_nvfp4_a4_unsupported(const Weight&, Tensor&, Nvfp4W4a4Workspace, std::int32_t,
                                        cudaStream_t) {
    throw std::logic_error("nvfp4 W4A4 routes require sm_100 or newer");
}

template <Nvfp4GeometryId>
constexpr Nvfp4A4Route nvfp4_a4_tma_route() {
    return {launch_nvfp4_a4_unsupported, Nvfp4ScaleLayout::Tiled};
}

template <class, class>
constexpr Nvfp4A4Route nvfp4_a4_mma_route() {
    return {launch_nvfp4_a4_unsupported, Nvfp4ScaleLayout::RowMajor};
}
#else
template <Nvfp4GeometryId Geometry>
constexpr Nvfp4A4Route nvfp4_a4_tma_route() {
