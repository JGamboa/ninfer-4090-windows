...
// This geometry's brackets were retuned on a 5090 up to 16 warps/CTA. The fork's original
// W8LinearSmallTProductionSchedule override for this geometry (35B MoE routed projection) folded
// every bracket onto a flat 4-warp K-split instead of inheriting the wider sm_120a schedule, and
// this preserves that same conservative choice across upstream's now per-bracket dispatch; it has
// not been independently re-tuned against these new brackets on sm_89.
using C4  = Q8KSplitSchedule<4, 8, 2, Access::Direct, Cache::cg, Cache::cg, Stage::RuntimeActive>;
using C8  = Q8KSplitSchedule<4, 8, 2, Access::Shared, Cache::ca, Cache::cg, Stage::RuntimeActive>;
...
using C40 = Q8KSplitSchedule<4, 40, 2, Access::Shared, Cache::cg, Cache::cg, Stage::ActiveOnly>;
...
using C64 = Q8KSplitSchedule<4, 64, 2, Access::Shared, Cache::ca, Cache::cg, Stage::RuntimeActive>;
#else
