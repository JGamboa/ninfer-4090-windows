...
// One CTA per token: the 256-thread block is the fastest choice only while the whole launch
// is resident. Six of these CTAs fit per SM, so the wave ends at six times the device SM
// count; past that the narrower full-chunk block wins.
