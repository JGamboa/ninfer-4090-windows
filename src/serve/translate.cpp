...
ResolvedPromptSemantics resolve_prompt_semantics(const GenerationRequest& request,
    ...
    if (effort) {
        ...
        case RequestedReasoningEffort::None:
            ...
            break;
        // The Qwen3.8 chat template only implements low/medium/xhigh and raises on anything
        // else, while coding agents (Claude Code, OpenCode) routinely send `high`. Round the
        // unsupported tiers to the nearest implemented one instead of failing the request.
