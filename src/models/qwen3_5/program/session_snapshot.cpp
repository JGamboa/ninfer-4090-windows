...
// chars. The full-ledger form is the session_digest clients see; a checkpoint's digest is the
// same hash over the prefix its frontier covers (continuation_checkpoints below).
...
// Deterministic across processes on one endianness, which snapshot compatibility already
// requires.
...
qwen3_5::RetainedSessionSnapshot
ProgramImpl::save_continuation(const ContinuationHandle& continuation,
    ...
    qwen3_5::RetainedSessionSnapshot snapshot;
    ...
    for (std::size_t index = 0; index < unique_states.size(); ++index) {
        ...
        if (state_store->residency(image) == StateReplicaResidency::HostOnly) {
            const qwen3_5::HostStateImageConstView view = state_store->host_view(image);
...
ContinuationHandle
ProgramImpl::restore_continuation(std::span<const std::uint8_t> snapshot,
                                  std::string_view model_binding) {
    if (has_context_transaction() || pending_transaction_) {
    ...
    const DeviceKVPagePool& text_pool        = text_kv_pages->physical_pool();
    const HostKVPageLayout text_layout       = plan_host_kv_page_layout(text_pool.geometry());
    qwen3_5::PagedKVCache* backend           = backend_kv_cache();
    std::optional<HostKVPageLayout> backend_layout;
    if (backend != nullptr) {
        backend_layout = plan_host_kv_page_layout(backend->page_pool().geometry());
    }
    ...
    for (const TokenId id : ledger) {
        if (id < 0 || id >= static_cast<TokenId>(qwen3_5::kTokenDomain)) {
    ...
    try {
        ...
        const auto build_address =
            ...
            try {
                addresses.activate(*address, page_count, *free_row, device.stream);
