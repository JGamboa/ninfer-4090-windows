...
public:
    ...
    // (FNV-1a 64, session_snapshot.cpp). The v3 format persists the rewrite checkpoint and long
    // anchors beside the endpoint, so a restored continuation offers every surviving checkpoint
    // frontier that fit the server's state pools and anchor capacity, not just the endpoint.
    ...
    [[nodiscard]] qwen3_5::RetainedSessionSnapshot
    ...
    [[nodiscard]] qwen3_5::SessionSnapshotTraffic session_snapshot_traffic() const noexcept {
        return snapshot_traffic_;
    }

    qwen3_5::SessionSnapshotTraffic snapshot_traffic_;
