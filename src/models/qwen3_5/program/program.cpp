...
RetainedSessionSnapshot Program::save_continuation(const ContinuationHandle& continuation,
...
SessionSnapshotTraffic Program::session_snapshot_traffic() const noexcept {
...
// Spelled out rather than defaulted: MSVC rejects the defaulted noexcept move-assignment of a
// class whose only member is a unique_ptr to an incomplete type at this point.
AdmissionCandidate& AdmissionCandidate::operator=(AdmissionCandidate&& other) noexcept {
...
CapturePressureCandidate::operator=(CapturePressureCandidate&& other) noexcept {
    impl_ = std::move(other.impl_);
    return *this;
}
